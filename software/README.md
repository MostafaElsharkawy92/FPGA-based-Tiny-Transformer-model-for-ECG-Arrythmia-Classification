...
|--------|---------------------|--------------------------------|-----------|
| **Head Processing** | All 8 heads always active | Adaptive: avg 5.2/8 heads | 35% reduction |
| **Computation** | Fixed: 100% FLOPS | Input-dependent: ~65% FLOPS | 35% savings |
...
| **Parameters** | 6,648 | 6,752 (+104) | +1.6% overhead |
| **Accuracy** | 98.97% | 98.97% | Maintained |
| **Inference (CPU)** | 12.3 ms | 11.1 ms | 9.8% faster |
| **Memory** | 26 KB | 26.4 KB | Negligible |
| **Clinical Value** | Limited explainability | Head specialization insights | Medical AI compliance |

**Head Activation Analysis**:

Measured on MIT-BIH validation set (2,000 samples):

```
Arrhythmia Type    Active Heads (avg)    Most Active Heads    Pattern
─────────────────────────────────────────────────────────────────────────
Normal (N)         4.8 / 8               [1, 3, 7]            Simple features
Supravent (S)      5.1 / 8               [2, 4, 6]            P-wave focus
Ventricular (V)    6.2 / 8               [2, 5, 8]            QRS complex
Fusion (F)         6.8 / 8               [2, 4, 5, 6, 8]      Multi-feature
Unknown (Q)        5.9 / 8               [1, 2, 3, 5, 7]      Diverse
```

**Key Findings**:
- Normal beats activate fewer heads (simpler patterns)
- Complex arrhythmias (V, F) require more heads
- Heads specialize: Head 2 consistently active for abnormal rhythms
- Interpretability: Correlates with clinical ECG interpretation
...
Gate Statistics (Validation Set):
  Mean gate score:        0.65 ± 0.18
  Sparsity (< 0.3):       35% of heads
  Active heads/sample:    5.2 ± 1.1
  Head specialization:    0.73 (1.0 = perfect)
  Entropy:                2.1 bits (uniform = 3.0 bits)
```
...
2. **Expected Hardware Benefits**:
   - **Latency**: 35% reduction (from 56ms → ~37ms)
   - **Power**: 25% savings (inactive heads power-gated)
   - **Throughput**: ~27 inferences/sec (vs 17.6 baseline)
   - **Energy**: ~1.9 mJ/inference (vs 2.5 mJ estimated)
